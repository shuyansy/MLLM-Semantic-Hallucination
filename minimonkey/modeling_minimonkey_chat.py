...
class MiniMonkeyChatModel(PreTrainedModel):
    ...
    def forward(
            ...
            SYS_LEN: int = 0,  
            IMG_LEN: int = 0, 
            bbox_tokens:Optional[torch.Tensor] = None,
    ...
    def chat(self, tokenizer, pixel_values, target_aspect_ratio, question, generation_config, use_scm=False, history=None, return_history=False,
             num_patches_list=None, bbox_tokens=None,IMG_START_TOKEN='<img>', IMG_END_TOKEN='</img>', IMG_CONTEXT_TOKEN='<IMG_CONTEXT>',
             verbose=False,):
    ...
    def generate(
        ...
        if pixel_values is not None:
            ...
            # find the starting token idx of image and question
            img_token_id = 92544       # <img>
            img_end_token_id = 92545   # </img>

            img_start_idx = ((input_ids == img_token_id).nonzero(as_tuple=True)[0]).item()
            qs_start_idx = ((input_ids == img_end_token_id).nonzero(as_tuple=True)[0]).item()
